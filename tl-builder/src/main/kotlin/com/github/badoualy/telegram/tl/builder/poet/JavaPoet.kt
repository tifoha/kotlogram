...
import com.github.badoualy.telegram.tl.builder.OUTPUT_TEST
import com.github.badoualy.telegram.tl.builder.hex
import com.github.badoualy.telegram.tl.builder.javaEscape
import com.github.badoualy.telegram.tl.builder.lCamelCase
import com.github.badoualy.telegram.tl.builder.parser.TLAbstractConstructor
import com.github.badoualy.telegram.tl.builder.parser.TLConstructor
import com.github.badoualy.telegram.tl.builder.parser.TLDefinition
import com.github.badoualy.telegram.tl.builder.parser.TLMethod
import com.github.badoualy.telegram.tl.builder.parser.TLParameter
import com.github.badoualy.telegram.tl.builder.parser.TLType
import com.github.badoualy.telegram.tl.builder.parser.TLTypeAny
import com.github.badoualy.telegram.tl.builder.parser.TLTypeConditional
import com.github.badoualy.telegram.tl.builder.parser.TLTypeFlag
import com.github.badoualy.telegram.tl.builder.parser.TLTypeFunctional
import com.github.badoualy.telegram.tl.builder.parser.TLTypeGeneric
import com.github.badoualy.telegram.tl.builder.parser.TLTypeRaw
...
object JavaPoet {
    ...
    val constructorClassNameMap =
        HashMap<TLConstructor, ClassName>() // Map a constructor to it's className to use when building context

    val emptyConstructorAbstractedMap =
        HashMap<TLConstructor, Boolean>() // Map a constructor to true if it's abstracted for an "Empty" constructor only
    ...
    private fun writeConstructors() {
        ...
        val nonAbstractedConstructors =
            constructors.filter { typeOccurrences[it.tlType]!! == 1 } // No need for abstraction
        ...
        for (abstractType in abstractedConstructors.map { it.tlType }.distinct()) {
            ...
            val abstractEmptyConstructor =
                typeConstructors.size == 2 && typeConstructors.map { it.name.endsWith("empty", true) }.contains(true)
            ...
            val commonParameters =
                typeConstructors.map { it.parameters }.reduce { l1, l2 -> ArrayList(l1.intersect(l2)) }
            abstractConstructors.add(
                TLAbstractConstructor(
                    abstractType.name,
                    commonParameters,
                    abstractType,
            ...
            typeConstructors.flatMap { it.parameters }.filter { commonParameters.contains(it) }
        ...
        abstractConstructors.forEach { constructor ->
            typeClassNameMap.put(
                constructor.tlType,
                ClassName.get(packageName(constructor.name), className(constructor.name, abstract = true))
            )
        }
        abstractedConstructors.forEach { constructor ->
            constructorClassNameMap.put(
                constructor,
                ClassName.get(packageName(constructor.name), className(constructor.name))
            )
        }
        ...
        nonAbstractedConstructors.forEach { constructor ->
            writeClassToFile(
                JavaPoet.packageName(constructor.name),
        ...
        abstractedConstructors.forEach { constructor ->
            writeClassToFile(
                JavaPoet.packageName(constructor.name),
                generateConstructorClass(constructor, typeClassNameMap[constructor.tlType]!!)
            )
        }
    }
    ...
    private fun writeMethods() {
        ...
        apiWrappedClazz.addMethod(
            MethodSpec.methodBuilder("executeRpcQuery")
    ...
    private fun writeContext() {
        val contextClazz = TypeSpec.classBuilder(TL_API_CONTEXT)
            ...
            .addAnnotation(
                AnnotationSpec.builder(SuppressWarnings::class.java).addMember("value", "\"unused\"").build()
            )
            .superclass(TYPE_TL_CONTEXT)

        contextClazz.addMethod(
            MethodSpec.constructorBuilder().addModifiers(Modifier.PUBLIC)
                .addStatement("super(\$L)", contextConstructors.size).build()
        )
        ...
        val methodBuilder =
            MethodSpec.methodBuilder("init").addModifiers(Modifier.PUBLIC).addAnnotation(Override::class.java)
    ...
    private fun writeTestContext() {
        ...
        contextClazz.addMethod(
            MethodSpec.constructorBuilder().addModifiers(Modifier.PUBLIC).addStatement("super(\$L)", testContext.size)
                .build()
        )
    ...
    private fun generateAbstractConstructorClass(constructor: TLAbstractConstructor): TypeSpec {
        ...
        constructors.forEach {
            clazz.addJavadoc(
                "<li>{@link ${
                    constructorClassNameMap[it].toString().substringAfterLast('.')
                }}: ${it.name}#${hex(it.id)}</li>\n"
            )
        }
        clazz.addJavadoc("</ul>\n\n")
        ...
        if (constructor.abstractEmptyConstructor) {
            ...
            clazz.addMethod(
                MethodSpec.methodBuilder("isNotEmpty")
                    .addModifiers(Modifier.PUBLIC, Modifier.ABSTRACT)
                    .returns(TypeName.BOOLEAN)
                    .build()
            )
            ...
            clazz.addMethod(
                MethodSpec.methodBuilder("getAs" + nonEmptyConstructor.name.split(".").last().uCamelCase())
                    .addModifiers(Modifier.PUBLIC)
            ...
            if (constructor.name.startsWith("input", true)) {
                clazz.addMethod(
                    MethodSpec.methodBuilder("newEmpty")
                        .addModifiers(Modifier.PUBLIC, Modifier.STATIC)
                ...
                clazz.addMethod(
                    MethodSpec.methodBuilder("newNotEmpty")
                        .addModifiers(Modifier.PUBLIC, Modifier.STATIC)
                        .returns(constructorClassNameMap[nonEmptyConstructor])
    ...
    private fun generateConstructorClass(constructor: TLConstructor, superclass: ClassName = TYPE_TL_OBJECT): TypeSpec {
        ...
        if (emptyConstructorAbstractedMap.getOrDefault(constructor, false)) {
            ...
            if (!constructor.name.endsWith("empty", true)) {
                clazz.addMethod(
                    MethodSpec.methodBuilder("getAs" + constructor.name.split(".").last().uCamelCase())
                        .addModifiers(Modifier.PUBLIC, Modifier.FINAL)
                        .addAnnotation(Override::class.java)
    ...
    private fun generateMethodClass(method: TLMethod): TypeSpec {
        ...
        val deserializeResponseMethod = MethodSpec.methodBuilder("deserializeResponse")
            ...
            .addAnnotation(
                AnnotationSpec.builder(SuppressWarnings::class.java)
                    .addMember("value", """{"unchecked", "SimplifiableConditionalExpression"}""").build()
            )
        ...
        if (method.tlType is TLTypeAny) {
            ...
            // Delegate
            deserializeResponseMethod.addStatement(
            ...
            deserializeResponseMethod.addStatement("final \$T response = readTLObject(stream, context)", TYPE_TL_OBJECT)
                ...
                .beginControlFlow(
                    ...
                    if (responseType is ParameterizedTypeName) responseType.rawType else responseType
                )
                .addStatement(
                    "throw new \$T(\"Incorrect response type, expected \" + getClass().getCanonicalName() + \", found \" + response.getClass().getCanonicalName())",
        ...
        apiWrappedMethod = MethodSpec.methodBuilder(method.name.lCamelCase())
            ...
            .addStatement("return (\$T) executeRpcQuery(new \$T(\$L))",
                ...
                if (method.parameters.isNotEmpty())
                    method.parameters.filterNot { it.tlType is TLTypeFlag }.map { it.name.lCamelCase().javaEscape() }
                        .joinToString(", ")
                else ""
            )
    ...
    private fun generateClassCommon(
        ...
        if (id != null) {
            ...
            clazz.addField(
                FieldSpec.builder(TypeName.INT, "CONSTRUCTOR_ID", Modifier.PUBLIC, Modifier.STATIC, Modifier.FINAL)
        ...
        val equalsMethod = MethodSpec.methodBuilder("equals")
            ...
            .addAnnotation(
                AnnotationSpec.builder(SuppressWarnings::class.java)
                    .addMember("value", "\$S", "PointlessBooleanExpression").build()
            )
            .returns(TypeName.BOOLEAN)
                ...
                for (parameter in condBoolean) {
                    computeFlagsMethod.addStatement(
                        "" + parameter.name.lCamelCase()
                            .javaEscape() + " = (flags & ${(parameter.tlType as TLTypeConditional).pow2Value()}) != 0"
                    )
                }
            }
        ...
        if (id != null) {
            ...
            // _constructor
            clazz.addField(
                FieldSpec.builder(String::class.java, "_constructor")
            ...
            // getConstructorId
            clazz.addMethod(
                MethodSpec.methodBuilder("getConstructorId")
                    .addAnnotation(Override::class.java)
                    .addModifiers(Modifier.PUBLIC)
                    .returns(TypeName.INT)
    ...
    private fun getType(type: TLType, boxedInTl: Boolean = false): TypeName = when (type) {
        is TLTypeFunctional -> ParameterizedTypeName.get(
            TYPE_TL_METHOD,
            TypeVariableName.get("T")
        ) // ex: InitConnection
